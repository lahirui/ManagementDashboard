 ...
 */
        ...
        public DataTable getCMEBundlingData(string factoryName)
        {
            ...
            sqlCommand.CommandText= "SELECT Factory,(CASE WHEN Code='BUN' THEN 'BUNDLING'  WHEN Code='SEW' THEN 'SEWING' WHEN Code='FIN' THEN 'FINISHING' END) AS Area, " +
                                       ...
                                       "SUM(ISNULL(WIP,0)) as 'WIP', " +
                                       "SUM(ISNULL(OUTQ,0)) as 'OUTQ', " +
                                       "SUM(ISNULL(C_Out_Achieved_Minutes, 0)) AS 'CoutAchievedMinutes', "+
                                       "SUM(ISNULL(Clocked_Minutes, 0)) AS 'CoutClockedMinutes', " +
                                       "SUM(ISNULL(L_OUT_Achieved_Minutes, 0)) AS 'LOutAchievedMinutes', " +
                                       "SUM(ISNULL(Absent_Employees, 0)) AS 'AbsentEmployees', " +
                                       "SUM(ISNULL(Registered_Employees, 0)) AS 'RegisteredEmployees' " +
                                "FROM ManagementDashboard " +
        ...
        public DataTable getCMESewingData(string factoryName)
        {
            ...
            sqlCommand.CommandText = "SELECT (CASE WHEN Code='BUN' THEN 'BUNDLING'  WHEN Code='SEW' THEN 'SEWING' WHEN Code='FIN' THEN 'FINISHING' END) AS Area, Team, " +
                                           ...
                                           "SUM(ISNULL(Absent_Employees, 0)) AS 'AbsentEmployees', " +
                                           "SUM(ISNULL(Registered_Employees, 0)) AS 'RegisteredEmployees', " +
                                           "SUM(ISNULL(ILQDefects, 0)) AS 'ILQDefects', "+
									       "SUM(ISNULL(ILQPiecesChecked, 0)) AS'ILQPiecesChecked', "+
									       "(SUM(ISNULL(AQLPiecesChecked, 0)) - SUM(ISNULL(AQLPiecesFailed, 0))) AS 'PassedGarments', "+
									       "SUM(ISNULL(AQLPiecesChecked, 0)) AS 'AQLPiecesChecked' "+
                                    "FROM ManagementDashboard " +
